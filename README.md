...
| **学习档案** | 贴 GitHub 链接自动读取 README、依赖配置、目录与常见入口文件，再以固定模板生成项目架构、0→1→100 开发路线、学习证据与 Upwork 客户需求映射；也支持本地 / 教程项目手动建档 |
...
- **学习档案也只在本机**：项目、AI 解剖与学习记录保存在 `data/profile/memory.db`；GitHub 导入会在本机按需读取 README、依赖配置、目录与少量常见入口文件，AI 只根据这些可见证据和你的补充分析，不会假装完整读过源码
